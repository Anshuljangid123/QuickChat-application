...
const RightSidebar = () => {
    ...
    if (messages?.length) {
        const imgs = messages
        .filter(msg => msg.image)   // only keep messages with an image
        .map(msg => msg.image);     // extract just the image URLs

        setMessageImages(imgs);       // update state
    } else {
        setMessageImages([]);         // clear if no messages
    }
    }, [messages]);
