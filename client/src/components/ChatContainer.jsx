    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
        ...
        <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
            {messagesDummyData.map((msg ,index) => (
                ...
                <div className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f571ff10f3cd28382f094' && 'flex-row-reverse'}`} key={index}>

                    {msg.image ? (
                        <img src={msg.image} alt='' className='max-w-[230px] border border-gray-700 rounded-lg overflow-hidden mb-8'/>
                    ) : (
                        <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${msg.senderId === '680f571ff10f3cd28382f094' ? 'rounded-br-none' : 'rounded-bl-none'} `}>{msg.text}</p>
                    )}

                    <div className='text-center text-xs'>
                        <img src={msg.senderId === '680f571ff10f3cd28382f094' ? assets.avatar_icon : assets.profile_martin} className='w-7 rounded-full'/>
